...
# ╔═╡ 6008e10f-6e50-493e-9895-137826c24ec6
using StatsPlots
...
begin
	@model function dlm_model(y, K)
	    ...
	    x[1] ~ MvNormal(zeros(K), Matrix{Float64}(I, K, K))
	    y[:, 1] ~ MvNormal(C * x[1], inv(R_inv))
	
	    # Dynamics and observations
	    for t in 2:T
	        x[t] ~ MvNormal(A * x[t-1], Matrix{Float64}(I, K, K))
	        y[:, t] ~ MvNormal(C * x[t], inv(R_inv))
	    end
	end
end
...
begin
	...
	chain = sample(model, sampler, 1100)
	chain = chain[100:end]
end

# ╔═╡ cfd3fc64-498c-4fae-b9f2-18fbb7244b7b
md"""
Solving A, C, R using MCMC
"""
...
# ╔═╡ 1786a929-3b03-48df-92ec-976128cf447d
md"""
Extract relevent model parameters from MCMC Chain
"""

# ╔═╡ cb91292e-8ba2-46fa-9bf0-f1747e96d181
begin
	params = names(chain)  # get the names of the parameters

	A_params = params[1:min(4, end)]
	
	A_chain = chain[:, A_params, :]

	C_params = params[5:min(8, end)]

	C_chain = chain[:, C_params, :]

	ρ_params = params[9:min(10, end)]

	ρ_chain = chain[:, ρ_params, :]
end;

# ╔═╡ dfdbec49-1749-4c29-bbe2-ea6a06ee8c4f
plot(A_chain)

# ╔═╡ 6a3d0e62-665d-4544-b982-3927084f1d73
plot(C_chain)

# ╔═╡ 8d96b556-df85-4394-872f-216b0c0832cb
plot(ρ_chain)

# ╔═╡ 2d1cdc6f-c207-44dc-b5f4-797208dbea3f
md"""
Extract hidden state x
"""

# ╔═╡ 218a67aa-4a9e-44df-ba49-562720128abe
begin
	x_samples = Array{Float64}(undef, length(chain), 100, 2) # preallocate an array

	for t in 1:100
	    for d in 1:2
	        param_name = "x[$t][$d]"
	        x_samples[:, t, d] = chain[param_name]
	    end
	end
...
# ╔═╡ 9d41acf2-212d-4b3a-abc9-5c19f67a5600
x_s = mean(x_samples, dims=1)[1, :, :]

# ╔═╡ 7fba2b6c-26ea-424a-a602-07a23c5f53f9
md"""
Ground-truth
"""

# ╔═╡ 0f1e3574-cd9b-46b2-a73a-e86b93d20acf
x_true

# ╔═╡ 419ddba3-9a74-48a8-8a10-9869ae8ca8dd
...
# ╔═╡ ae6a4187-8e0e-446c-b54a-e4f2cc6ed6c5
error_metrics(x_true, x_s')
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "63b51c1263fcf623497eda7b1db15792b03d5d27"
