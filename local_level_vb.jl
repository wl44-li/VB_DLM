...
function forward_ll(y, a, c, E_τ_r, E_τ_q, priors::Priors_ll)
    ...
    return μ_f, σ_f2, log_z, fs, ss
end

# ╔═╡ 71789300-2e64-49af-8f29-f5d9c6f1a371
function forward_dlm(y, a, c, E_τ_r, E_τ_q, priors::Priors_ll)
    T = length(y)
...
let
	...
	mm, cc, _, _, zz = forward_dlm(y, 1.0, 1.0, E_τ_r, E_τ_q, hpp)
	cc, σs, σs_ll, zz, log_z
end
...
function vb_e_ll(y, E_τ_r, E_τ_q, priors::Priors_ll)
    # Forward pass (filter)
    μs_f, σs_f2, log_Z, _, _ = forward_ll(y, 1.0, 1.0, E_τ_r, E_τ_q, priors)
...
let
    ...
    μs_s, σs_s2, _, _, _ = backward_ll(μs_f, σs_f2, 1/q, hpp_ll)
	
	println("\nlatent x error: " , error_metrics(x_true, μs_s))
	lower_bound = μs_s - 1.96 .* sqrt.(σs_s2)
	upper_bound = μs_s + 1.96 .* sqrt.(σs_s2)
	
	# Create time variable for x-axis
	time = 1:length(μs_s)
	# Plot the smoothed means and the 95% CI
	plot(time, μs_s, ribbon=(μs_s-lower_bound, upper_bound-μs_s), fillalpha=0.5,
	     label="95% CI", linewidth=2, xlabel="Time", ylabel="Latent State")
	plot!(time, x_true, label="true x")
	title!("95% CI for Latent States")
end
...
let
	...
	plot(time, μs_s, ribbon=(μs_s-lower_bound, upper_bound-μs_s), fillalpha=0.5,
	     label="95% CI", linewidth=2, xlabel="Time", ylabel="Latent State")
	plot!(time, x_true, label="true x")
	title!("95% CI for Latent States (hyper-param update)")
end

# ╔═╡ 244683bd-661f-4de1-9cc7-1d4f07b2d2a6
let
	hpp_ll = Priors_ll(0.01, 0.01, 0.01, 0.01, 0.0, 1.0)
	@time r, q = vb_ll(y, hpp_ll, 69)

	μs_f, σs_f2, _, fs, ss = forward_ll(y, 1.0, 1.0, 1/r, 1/q, hpp_ll)
    μs_s, σs_s2, _, _, _ = backward_ll(μs_f, σs_f2, 1/q, hpp_ll)
	
	println("\ny pred error: " , error_metrics(y, fs))
	lower_bound = fs - 1.96 .* sqrt.(ss)
	upper_bound = fs + 1.96 .* sqrt.(ss)
	
	# Create time variable for x-axis
	time = 1:length(fs)
	
	# Plot the smoothed means and the 95% CI
	plot(time, fs, ribbon=(fs-lower_bound, upper_bound-fs), fillalpha=0.5,
	     label="95% CI", linewidth=2, xlabel="Time", ylabel="y")
	#plot!(time[2:end], y[2:end], label="y")
	plot!(y, label="y")
	title!("95% CI y")
end

# ╔═╡ 56603e7b-0b57-4341-b3b3-70f0e575e1d6
md"""
Y-pred with hyper-param update
"""

# ╔═╡ 961c46dd-1795-4054-b468-6a565cfd9b75
let
	hpp_ll = Priors_ll(0.01, 0.01, 0.01, 0.01, 0.0, 1.0)
	@time r, q = vb_ll_c(y, hpp_ll, true, 50)
	...
	#plot!(time[2:end], y[2:end], label="y")
	plot!(time, y, label="y")
	title!("95% CI y (hyper-param update)")
end
...
let
	...
	println("\nend chain x sample error ", error_metrics(x_true, x_ss[end,: ]))
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "d92895eb7614323aa7985e226f9bcfde9057ff05"
